Pod::Spec.new do |s|
  s.name             = 'facebook_flutter_sdk'
  s.version          = '0.24.0'
  s.summary          = 'Flutter plugin for Facebook Analytics and App Events'
  s.description      = <<-DESC
Flutter plugin for Facebook Analytics and App Events
                       DESC
  s.homepage         = 'https://github.com/oddbit/flutter_facebook_flutter_sdk'
  s.license          = { :file => '../LICENSE' }
  s.author           = { 'Oddbit Team' => 'opensource@oddbit.id' }
  s.source           = { :path => '.' }
  s.source_files = 'facebook_flutter_sdk/Sources/facebook_flutter_sdk/**/*.{swift}'
  s.static_framework = true
  s.dependency 'Flutter'
  s.swift_version       = '5.0'
  s.ios.deployment_target = '12.0'

  # Do not specify PATCH version of FBSDKCoreKit. See README file for explanation
  # https://github.com/oddbit/flutter_facebook_flutter_sdk#dependencies-on-facebook-sdk
  s.dependency 'FBSDKCoreKit', '~> 18.0'
end
